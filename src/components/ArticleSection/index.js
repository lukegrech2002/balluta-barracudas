...
const ArticleSection = ({ articles }) => (
  <>
    <div
      className={tw(
        'p-10 grid grid-cols-1 xs:grid-cols-1 sm:grid-cols-2 md:grid-cols-1 lg:grid-cols-2 xl:grid-cols-2 gap-5',
      )}
    >
      <ul>
        {articles &&
    ...
    {articles && (
      <Link href="/articles">
        <a className={tw('text-sm mr-10 float-right text-default-burgundy')}>View Articles {'>'}</a>
      </Link>
    )}
  </>
);
